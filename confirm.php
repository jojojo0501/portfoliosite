<?php 
    ...
    $body__toOwn = <<< EOM
      ...
      EOM;
		...
		// 自分のメールアドレスを変数fromEmailに格納
		$fromEmail = "sarukiti0418@outlook.jp";
		// 自分の名前を変数fromNameに格納
		$fromName = "【自動返信】金子丈太";
		// ヘッダ情報を変数headerに格納する		
		$header = "From: " .mb_encode_mimeheader($fromName) ."<{$fromEmail}>";
		$header__toOwn = "From: " .mb_encode_mimeheader($subject__toOwn) ."<{$email}>";
		// メール送信を行う（相手）
		mb_send_mail($email, $subject, $body, $header);
    // メール送信を行う（自分）
    mb_send_mail($fromEmail, $subject__toOwn, $body__toOwn, $header__toOwn);
 		// サンクスページに画面遷移させる
